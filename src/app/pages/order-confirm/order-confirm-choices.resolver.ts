...
const STATUS_FILTERS = ["unpaid", "waiting"];

@Injectable({
  providedIn: "root",
})
export class OrderConfirmChoicesResolver implements Resolve<any> {
  constructor(private orderHistoryService: OrderHistoryService, private paymentMethodService: PaymentMethodService) {}
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> {
    return this.orderHistoryService.fetchAll({ payment_type: "manual_transfer", status: STATUS_FILTERS }).pipe(
