...
@Component({
  selector: "app-wishlist-pagination",
  template: `
...
export class WishlistPaginationComponent implements OnInit {
  ...
  calculateMaximumPageCount(): number {
    // pageSize count from 0, so that we need increment that value
    return Math.ceil(this.page.totalResults / this.getPageSize());
  }
}
